...
function getToolLabel(tool: { toolName: string; args?: Record<string, unknown> }): string {
  const args = tool.args as { command?: string; path?: string; new_path?: string } | undefined;
  ...
  if (tool.toolName === "file_manager") {
    const newFile = args?.new_path?.split("/").pop() ?? "";
    switch (args?.command) {
      case "delete": return `Deleting \`${filename}\``;
      case "rename": return `Renaming \`${filename}\` to \`${newFile}\``;
    }
  }
